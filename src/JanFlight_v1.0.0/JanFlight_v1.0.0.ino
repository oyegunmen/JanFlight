...
// Choose receiver protocol (Uncomment exactly one protocol)
#define USE_PPM_RX
//#define USE_PWM_RX
//#define USE_SBUS_RX
...
#include <Servo.h> // STM32 Native Core Servo Engine 

//========================================================================================================================//
//                                         3. USER-SPECIFIED VARIABLE DECLARATION                                         //
//========================================================================================================================//

// Failsafe values (If the radio signal is lost, the code defaults to these)
unsigned long channel_1_fs = 1000; // Throttle low (technically Off)
unsigned long channel_2_fs = 1500; // Aileron centered
unsigned long channel_3_fs = 1500; // Elevator centered
unsigned long channel_4_fs = 1500; // Rudder centered
unsigned long channel_5_fs = 2000; // Gear/Arming switch (High = Throttle Cut ON)
unsigned long channel_6_fs = 1500; // Aux

// IMU Filter Parameters (Tuned for 2kHz)

//Values between 0 and 1. A lower value smooths out motor vibration noise but adds latency. A value of 0.10 means the new reading is 10% new data, 90% old data.
...
float i_limit = 25.0;  // Maximum integrator buildup
float maxRoll = 30.0;  // Angle mode max roll (degrees)
float maxPitch = 30.0; // Angle mode max pitch (degrees)
float maxYaw = 160.0;  // Yaw max rate (deg/sec)

// PID Gains (Angle Mode) (Kp is the immediate reaction to an error. Ki builds up over time to fix continuous sagging (like uneven weight). Kd dampens the reaction to stop oscillations.)
...
// Based on STM32F405 Nomenclature

// Receiver Pins
const int PPM_Pin = PA8; // FlySky PPM Input

// ESC / Motor Pins (OneShot125)
const int m1Pin = PA0;
const int m2Pin = PA1;
const int m3Pin = PA2;
const int m4Pin = PA3;
...
unsigned long channel_1_raw, channel_2_raw, channel_3_raw, channel_4_raw, channel_5_raw, channel_6_raw; // Background hardware routines write to these asynchronously. The main loop safely copies and filters them into channel_x_pwm
...
float q0 = 1.0f, q1 = 0.0f, q2 = 0.0f, q3 = 0.0f; // Quaternions; prevents "Gimbal Lock", euler angles perform poorly
...
void setup() {
  ...
  // Get IMU error offset (Uncomment to calibrate, then write them into section 3 under IMU Error Offsets and re-comment) (Keep IMU on Flat Surface)
  // calculate_IMU_error(); 
...
// MCU runs these functions on repeat at 2 kHZ frequency
void loop() {
  // Track timing
  ...
  // Print Data (Uncomment one for troubleshooting)
  // printRadioData();
  // printRollPitchYaw();
  // printMotorCommands();

  // Prints Loop Time (only every 500 loops, 4 times in a second)
  /*static int printCounter = 0;
  if (++printCounter >= 2000) { 
    Serial.print("Loop Time: "); Serial.print(dt * 1000000); Serial.println(" µs"); 
    printCounter = 0; 
  }*/
  ...
  // Idle until 2kHz loop boundary is reached
  loopRate(2000); 
}
...
// communicating with the MPU6500 using I2C protocol without needing external library.
void IMUinit() {
  Wire.begin();
...
void getIMUdata() {
  ...
  int16_t Temp = Wire.read() << 8 | Wire.read(); // Discard temp
...
// Let the Mahony filter chew on the data for a few seconds so orientation is stable before arming.
...
void getDesState() {
  thro_des = constrain((channel_1_pwm - 1000.0)/1000.0, 0.0, 1.0); 
  roll_des = constrain((channel_2_pwm - 1500.0)/500.0, -1.0, 1.0) * maxRoll;
  pitch_des = constrain((channel_3_pwm - 1500.0)/500.0, -1.0, 1.0) * maxPitch;
  yaw_des = constrain((channel_4_pwm - 1500.0)/500.0, -1.0, 1.0) * maxYaw;
}
...
void controlANGLE() {
  ...
  if (channel_1_pwm < 1060) integral_roll = 0; // Prevent windup on the ground
...
// Scale motor mixer outputs into pure OneShot125 microsecond ranges
void scaleCommands() {
...
// If CH5 is high (or flight isn't armed), slam motor outputs to 120us (off for oneshot)
void throttleCut() {
...
// Pulls designated pins HIGH, waits for microsecond limits, and pulls LOW.
void commandMotors() {
  int wentLow = 0;
...
void getCommands() {
  ...
  channel_5_pwm = channel_5_raw; // Switches Don't need filter
...
// ---------------------------------------------------------
// This runs completely outside the main loop, so we don't miss any radio signal, it interrupt triggers, measures the time since the last trigger, and bins that time into the correct radio channel variable.
void getPPM() {
...
// ----------------------------------------------------------------
// Functions for Troubleshooting, Will add more in coming versions
// ----------------------------------------------------------------
void printRadioData() {
  if (current_time - print_counter > 10000) {
    ...
    Serial.print(" CH3: "); Serial.println(channel_3_pwm);
  }
}
...
void printMotorCommands() {
  if (current_time - print_counter > 10000) {
    print_counter = micros();
    Serial.print("M1: "); Serial.print(m1_command_PWM);
    Serial.print(" M2: "); Serial.println(m2_command_PWM);
  }
}