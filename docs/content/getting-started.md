...
## 4. Compile & Upload
Connect your STM32 board via USB. Select the correct COM port in the IDE, click the Upload button, and wait for the compilation to finish. The IDE will flash the binary directly to the board's internal memory.

## 5. Calibrate
With the board sitting perfectly flat on a level surface, uncomment the `calculate_IMU_error()` function in `setup()` and upload. Open the serial monitor to read your specific sensor offsets. Copy these numbers, hardcode them into your error variables at the top of the file, and re-comment the calibration function so it does not run on every boot.
